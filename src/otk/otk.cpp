  ...
  while (!valid_input) {
    ...
    if (index_valid) {
      ...
      if (is_parent_dir) {
        ...
        bool is_dir = std::filesystem::is_directory(files[file_index - 1]);
        if (is_dir) {
          ...
          if (is_odb_file) {
            ...
            std::cout << "File select is not an ODB file. Try again.\n";
          }
        }
      }
    } else {
...
void get_points_from_nodes(vtkPoints *points,
  ...
  for (int i = 0; i < num_nodes; ++i) {
    ...
    if (instance_type == odb_Enum::THREE_D) {
      ...
      node_map[node_label] = points->InsertNextPoint(node_coordinates[0],
